...
def pad_list(dir_path,locs,dir3,num_cams):
  ...
  items = locs
  while len(locs) <= num_cams:
    items.append('dummy')
  # Get the filenames for the items.
  file_names = []
  for ii in list(range(1,len(items))):
      file_names.append(dir_path + '/' + locs[ii] + '_' + dir3)
  return file_names
...
def parallel_image(devices):
    processes = []
    for i in range(len(devices)):
        command = "ffmpeg -f v4l2 -i " + devices[i] + " -vframes 1 output" + str(i) + ".jpg"
...
# Function to get the available devices. Add the variable "devices" and it returns "devices2" which is a list of only the devices that are present.
def check_devices_online(devices):
    # Make a list of devices that are present. 
    result = subprocess.run(['v4l2-ctl', '--list-devices'], stdout=subprocess.PIPE)
    output = result.stdout.decode('utf-8')
    devices_available = output.strip().split('\n')
    devices_available = [device.replace('\t', '') for device in devices_available]
    
    # Check if the programmed devices are present.
    devices_set = set(devices)
    devices_available_set = set(devices_available)
    devices2 = devices_set.intersection(devices_available_set)
    return devices2
devices2 = check_devices_online(devices)
# Check cameras available:
v4l2-ctl --list-devices

print("Pictures taken with webcams at: " + webcam1 + ", " + webcam2 + ", and " + webcam3)
