...
devices = ['/dev/video5','/dev/video7','/dev/video0','/dev/video2']   # List of all webcam devices
x = 1                                  # Temp for setting the number of images to take
iter_max = 20
disk_max = 85.0 # The minimum amount of free space that should be kept on the computer [0-100].
error = 'no'
i_columns = 2                     # The number of columns in the collage summary file.
j_rows = 2                        # The number of rows in the collage summary file.
...
def pad_list(dir_path,locs,dir3,num_cams):
  ...
  file_names = []
  for ii in list(range(0,len(items))):
...
def add_text_to_image(image_path, text):
    ...
    if os.path.isfile(image_path):
        ...
        # Select a font
        text_size = image.size[0]/32 # For smaller images the text size should be smaller too.
        font = ImageFont.truetype('Arial.ttf', int(text_size))
        # Get the size of the text
        text_width, text_height = draw.textsize(text, font=font)
        # Calculate the position of the bottom left corner of the image
        x = 0
        y = height - text_height
        # Add the shadow text to the image
...
while 'true' == 'true':
    ...
    dir2 = now.strftime('%d')
    dir3 = now.strftime('%Y-%m-%d_%H-%M-%S')+'.jpg'
    ...
    if pic == 'yes':
        ...
        for ii in list(range(0,len(locs))):
            text_label = locs[ii] + ' ' + now_time
            add_text_to_image(locs_new[ii], text_label)
        # Make a collage with all the images from this timepoint.
        print('making collage')
        collage = create_collage(locs_new,i_columns,j_rows)
